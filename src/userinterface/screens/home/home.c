 ...
 * styling. Now includes gesture support for notifications screen.
 *
 * @license GNU v3
...
void home_screen_event(lv_event_t* event)
{
    ...
    if (event_code == LV_EVENT_GESTURE) {
        ...
        if (dir == LV_DIR_TOP) {
            ...
            // Create a smooth slide transition from bottom to top
            lv_screen_load_anim(menu_screen, LV_SCR_LOAD_ANIM_MOVE_TOP, 300, 0, false);
        }

        // Check for left-to-right gesture to open notifications.
        else if (dir == LV_DIR_RIGHT) {
            // Initialize notifications screen if not already done.
            if (!lv_obj_is_valid(notifications_screen)) {
                notifications_screen_init();
            }
            // Load notifications screen with slide transition
            notifications_screen_load();
        }
    }
}
