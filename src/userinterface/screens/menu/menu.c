...
#include "userinterface/screens/settings/settings.h"
...
int register_application(lv_obj_t** screen, init_fn_t init_fn, char* name)
{
    ...
    applications[application_count].init_fn = init_fn;
...
static void menu_item_event_handler(lv_event_t* event)
{
    ...
    if (code == LV_EVENT_CLICKED) {
        ...
        if (*app_index < application_count) {
            if (applications[*app_index].init_fn != NULL && !lv_obj_is_valid(*applications[*app_index].screen)) {
                applications[*app_index].init_fn();
            }

            if (lv_obj_is_valid(*applications[*app_index].screen)) {
                // Switch to the selected application screen
                LOG_DBG("Switching to application: %s", applications[*app_index].name);
                lv_screen_load_anim(*applications[*app_index].screen, LV_SCR_LOAD_ANIM_MOVE_LEFT, 300, 0, false);
            }
        }
    }
}
...
void menu_screen_init()
{
    ...
    register_application(&settings_screen, settings_screen_init, "Settings");
    register_application(NULL, NULL, "Stopwatch");
    register_application(NULL, NULL, "Weather");
    register_application(NULL, NULL, "Music");
